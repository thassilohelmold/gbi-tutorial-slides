...
\begin{frame}{Wahr oder Falsch?}
	...
	\begin{itemize}
		\truefalseQ{1}{myGreen}{$R(y,g(x,y))$ ist präd.log. syntaktisch korrekt.}{W}
		\truefalseQ{3}{myRed}{$f(S(x))$ ist präd.log. syntaktisch korrekt.}{F \\ Eine Relation kann nicht innerhalb einer Funktion auftauchen, da sie kein Term, sondern eine atomare Formel darstellt.}
		\truefalseQ{5}{myRed}{\enquote{Nicht alle Kinder spielen nicht} $\equiv \forall x(child(x)\to play(x)) $}{F\\ Es wird nur ausgesagt, dass es mindestens ein Kind gibt, das spielt.}
