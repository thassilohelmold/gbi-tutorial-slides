...
\begin{frame}
	...
	\begin{itemize}
		...
		\FalseQuestionE{Wenn wir $A$ schreiben, ist das immer eine Menge}{Das muss man explizit angeben}
		\FalseQuestionE{Ein Beispiel ersetzt einen Beweis.}{Mit einem Beispiel kann man eine Behauptung widerlegen, es zeigt aber nicht die Allgemeingültigkeit}
		% Modified by DJ (a more subtle version):
		\FalseQuestionE{$\setsize{M} = M$ für M \subseteq \N}{$\; \setsize{M}$ ist die \textbf{Anzahl} der Elemente in $M$}
	\end{itemize}
...
\begin{frame}
	\begin{itemize}
		...
		\FalseQuestionE{$01^* = \{\varepsilon, 01, 0101, ...\}$}{$\quad 01^*$ gibt es nicht, denn $01 \neq \{01\}$}
		\TrueQuestionE{Es gibt Sprachen L, für die gilt $\varepsilon \in L^+$}{}
