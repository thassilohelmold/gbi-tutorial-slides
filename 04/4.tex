...
\begin{frame}
	...
	\begin{itemize}
		\truefalseQ{1}{myGreen}{Beweise fängt man mit der Behauptung an}{W}
		\truefalseQ{3}{myRed}{Wenn wir $A$ schreiben, ist das immer eine Menge}{F \\Das muss man explizit angeben}
		\truefalseQ{5}{myRed}{Ein Beispiel ersetzt einen Beweis.}{F \\ Mit einem Beispiel kann man eine Behauptung widerlegen, es zeigt aber nicht die Allgemeingültigkeit}
		\truefalseQ{7}{myRed}{$\setsize{M} = M$}{F $\; \setsize{M}$ ist die \textbf{Anzahl} der Elemente in $M$}
	\end{itemize}
...
\begin{frame}
	\begin{itemize}
		\truefalseQ{1}{myRed}{Jede Sprache enthält Wörter}{F $\quad \emptyset$ ist auch eine gültige Sprache}
		\truefalseQ{3}{myRed}{$01^* = \{\varepsilon, 01, 0101, ...\}$}{F $\quad 01^*$ gibt es nicht, denn $01 \neq \{01\}$}
		\truefalseQ{5}{myGreen}{Es gibt Sprachen L, für die gilt $\varepsilon \in L^+$}{W $\quad L = \{\varepsilon\}$}
		\truefalseQ{7}{myRed}{$L^+ = L^* \setminus L^0$}{F $\quad$ Das gilt nicht, wenn $\varepsilon \in L$}
