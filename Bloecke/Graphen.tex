...
\begin{frame}{Graphen}
	\begin{Definition}
		Ein \textbf{gerichteter Graph} ist ein Paar $G = (V, E)$ mit einer \emph{endlichen}, nicht leeren Menge an \textbf{Knoten} $V$ und einer Menge an \textbf{Kanten}\\
		$E \subseteq V \times V$.
	...
	\bigskip
	\begin{Definition}
		Ein \textbf{ungerichteter Graph} ist ein Paar $G = (V, E)$ mit einer \emph{endlichen}, nicht leeren Menge an \textbf{Knoten} $V$ und einer Menge an \textbf{Kanten}\\
		$E \subseteq \{ \{x,y\} \mid x \in V \aland y \in V \}$.
	\end{Definition} \pause
...
\begin{frame}{Beispiel}
	Wir betrachten den gerichteten Graphen $G = (V, E)$ mit $V = \{0,1,2,3,4,5\}$ und $E = \{(0,1), (1,0), (1,2), (3,4), (4,3) ,(4,5)\}$
	\bigskip
...
\begin{frame}{Lösung: Graphen Zeichnen}
	...
	$$V = \nZ_4 \qquad E = \{(0,1), (0,2), (0,3), (1,2), (1,3), (2,2), (2,3), (3,2)\}$$
...
\begin{frame}{Pfade und Wege}
	\begin{Definition}
		...
		Ein \textbf{Pfad / Weg} ist eine Folge von Knoten, die jeweils über Kanten im Graphen erreichbar sind. Also eine nichtleere Liste $$p = (v_0, v_1, \dots, v_n) \qquad (v_i, v_{i+1}) \in E$$
	\end{Definition}
	\pause
	Die Länge eines Pfades ist die Anzahl der Kanten ($|p| - 1$)
	\begin{Beispiel}
		Sei $G = (V, E)$ mit $V = \nZ_9, \quad E = V \times V$\\ \pause
...
\begin{frame}
	\frametitle{Aufgabe: Bäume (WS 2010) \stars{1}}
...
% Pufferaufgabe
\begin{frame}
	\frametitle{Aufgabe (WS 2009) \stars{4}}
	...
	Für $n \in \nN^+$ sei der Graph $G_n = (V_n , E_n )$ gegeben durch
	$$V_n =\{m \in \nN^+ \ | \ m \text{ teilt } n\}$$
	$$E_n =\{(k, m) \in V_n \times V_n \ | \ k \text{ teilt } m \text{ und } m/k  \text{ ist eine Primzahl} \}$$
	\begin{itemize}
		\item Zeichnen Sie $G_{12}$ , $G_{16}$ und $G_{30}$.
		\item Zeigen Sie: $$\forall \ n, m \in \nN^+: n \text{ teilt } m \implies G_n \text{ ist Teilgraph von } G_m$$.
	\end{itemize}
\end{frame}
...
\begin{frame}{Lösung}
	\textit{Zeigen Sie: $$\forall \ n, m \in \nN^+: n \text{ teilt } m \implies G_n \text{ ist Teilgraph von } G_m$$}\\[2em] \pause 
