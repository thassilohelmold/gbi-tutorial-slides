...
\begin{frame}{Reguläre Ausdrücke}
	Bislang: Beschreibung der Sprache $L$ eines endlichen Automaten aus einzelnen Mengen, Vereinigung, Konkatenation, \dots \\
	$$ L = (\{\word{ab}\} \cdot \{\word c, \word d\})^\ast \cup \{\word e\}$$
	\medskip \pause
	
	Jetzt: Ein Ausdruck $R$, um eine mit solchen Operationen \enquote{zusammengebaute} Sprache zu beschreiben.\\
	$$ R = \rx{(ab(c|d))*|e}$$
	
	Wir bezeichnen die mit $R$ beschriebene Sprache mit $\lang{R}$.\\
...
\begin{frame}{Reguläre Ausdrücke}
	...
	\begin{itemize}
		\item dem leeren Ausdruck $\rx O$ \qqquad $\lang{\rx{O}} = \emptyset$\pause
		\item den einzelnen Symbolen $x \in A$ \quad $\lang{x}=\{x\}$ \pause
		\item zwei regulären Ausdrücken $R_1$ und $R_2$ mit\\ 
			$\rx(R_1 R_2\rx)$ \qquad $\lang{R_1 R_2} = \lang{R_1} \cdot \lang{R_2}$\\
			$\rx(R_1\rx|R_2\rx)$ \qquad $\lang{R_1 \rx| R_2} = \lang{R_1} \cup \lang{R_2}$ \pause
		\item einem Stern $R\rx*$ \qquad $\lang{R\rx*} = \lang{R}^*$\pause
	\end{itemize} 
