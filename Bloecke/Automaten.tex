...
\begin{frame}{Mealy-Automat}
	...
	\begin{Definition}
		...
		\thassedaniel{\begin{itemize}[<+->]}{\begin{itemize}}
...
%DJ: I don't think there is time for this, esp. if you want to get to regexes...
\mycomment{
	\begin{frame}{Mealy-Automaten: Aufgaben}
		%\begin{enumerate}
		%\item 
		Malt einen Mealy-Automaten, der immer, nachdem er 4-mal ein \word a gelesen hat, \word{X!} ausgibt (und sonst nie etwas). Als Eingabe erhält der Automat nur Wörter aus $\set{\word a}^*$.
		%\item Erweitert den Automaten so, dass ein \word b ein \word a „neutralisieren“ kann (und damit die Ausgabe eines \word{X!} um eins verzögert). Sonst soll sich an der Ausgabe nichts ändern.  %Ich denke, das ist zu viel... zu wenig Zeit...
		%\end{enumerate}
		\pause
		% TODO Lösung
	\end{frame}	
