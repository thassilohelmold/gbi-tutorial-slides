...
\begin{frame}
	...
	\begin{itemize}
		...
		\item<2->[\emph{IV}] Für ein beliebig aber festes $n\in\nN_0$ sei $Num_b(w) für alle w mit \setsize{w} = n wohldefiniert und entspreche dem Zahlenwert.$ }
...
%Es bezeichne $\nZ$ die Menge der ganzen Zahlen. Gegeben sei eine Ziffernmenge $Z_{-2} = \{N, E\}$ mit der Festlegung $num_2 (N) = 0$ und $num_2 (E) = 1$. Wir definieren eine Abbildung $Num_{-2} : Z_{-2}^\ast \to \nZ$ wie folgt:
%	$$Num_{-2} (\varepsilon) = 0$$
...
%		\item Für welche Zahlen $x \in \nZ$ gibt es ein $w \in Z_{-2}^\ast$ mit $Num_{-2} (w) = x$?
%	\end{itemize}
%\end{frame}
%
%\begin{frame}{Lösung}
...
%	\end{table}
%	\pause
%	\textit{Für welche Zahlen $x \in \nZ$ gibt es ein $w \in Z_{-2}^\ast$ mit $Num_{-2} (w) = x$?} \\[1em]\pause
%	Für alle!
%\end{frame}
