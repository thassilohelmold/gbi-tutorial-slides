...
\begin{frame}
	...
	\begin{align*}
		...
		%\{1,2\} \only<1-3>{\ &? \ } \only<4->{&\subseteq}  \{1,2,3\} \\
		\{1,2\} \only<4->{&\subseteq}  \{1,2,3\} \\ %TODO: Handout line
		%\{1,2\} \only<1-4>{\ &? \ } \only<5->{&\nsubseteq} \{Hund, Katze, Maus\}
		\{1,2\} \only<5->{&\nsubseteq} \{Hund, Katze, Maus\} %TODO: Handout line
	\end{align*}
	\end{block} 
...
\begin{frame}{Mengengleichheit: Beispiel}
	\begin{itemize}
		\item<1-> Sei $ A $ und $M$ beliebige Mengen. Zeigen Sie das gilt 
		\begin{align*}
