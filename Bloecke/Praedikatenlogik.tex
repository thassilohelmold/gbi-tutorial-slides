...
\begin{frame}{Aufgabe: Syntaktische Fehler}
	...
	\begin{itemize}
		...
		\visible<2-|handout:2>{Konstanten sind keine Relationen!}
		\item $\word{S(c)} \aland \word{T(d)} \alimpl \word{S(R(c,d))}$\\
		\visible<3-|handout:2>{Innerhalb von Relationen können nur Terme stehen, keine Relation.}
		...
		\visible<5-|handout:2>{Relationen können nicht quantifiziert werden.}
		\item $\plall \word f \: \word{S(f)} \aland \plall \plx \: \word{g(R(x))}$\\
		\visible<6-|handout:2>{Innerhalb einer Funktion können nur Terme stehen, keine Relationen.\\
