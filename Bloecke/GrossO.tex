...
\begin{frame}{Asymptotisches Wachstum}
	\begin{Definition}
		Zwei Funktionen $f,g: \N_0 \to \R_0^+$ wachsen asymptotisch genauso schnell, wenn es zwei Konstanten $c, c' \in \R^+$ gibt, so dass gilt $$\exists n_0 \in \N_0: \ \forall n > n_0: \ c f(n) \leq g(n) \leq c'f(n) $$
...
\begin{frame}{Asymptotisches Wachstum}
	\begin{Definition}
		...
		$$g \preceq f \qquad \exists c \in \R^+ \ \exists n_0 \in \N_0 \ \forall n > n_0: \ g(n) \leq c f(n)$$
		$$g \succeq f \qquad \exists c \in \R^+ \ \exists n_0 \in \N_0 \ \forall n > n_0: \ g(n) \geq c f(n)$$
	\end{Definition} \pause
...
%	
%	Also gibt es ein $c$ in $\R^+$ (nämlich 2), so dass es ein $n_0 \in \N$ gibt (nämlich 1), so dass für alle $n > 1$ gilt: $$g(n) \leq c f(n)$$
%	\end{block}
%\end{frame}
%
%\begin{frame}{Polynome}
...
%	Dann müsste es ein $c' \in \R^+$ geben, so dass es ein $n_0 \in \N$ gibt, so dass für alle $n > n_0$ gilt: $$f(n) < c' g(n)$$ Wähle jetzt ein $n' > n_0$ so, dass $f(n'), g(n') \neq 0$ gilt. Insbesondere wäre dann $$\frac{f(n')}{g(n')} = n^2 + n \leq c$$ Dies ist ein Widerspruch
%	\end{block}
%\end{frame}
