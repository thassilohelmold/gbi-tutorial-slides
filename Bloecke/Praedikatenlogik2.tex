...
\begin{frame}{Variablenbelegungen, Auswertung}
	\begin{block}{Variablenbelegung $\beta \from \VPL \functionto D$}
		...
		Beispiel: \\
		\begin{itemize}
			\item $\beta(\word x) = 42$ und $\beta(\word y) = 1337$ \quad oder
			\item $\beta(\word x) = 2$ und $\beta(\word y) = 10000$ 
		\end{itemize}
	\end{block}
	\pause
...
\begin{frame}{Aufgabe}
	...
	Gebt eine Interpretation $(D_2, I_2)$, $D_2 \subseteq \N$, und eine Variablenbelegung $\beta_2$ so an, dass $val_{D_2, I_2, \beta_2}(G) = \W$ gilt.\\
	\smallskip
