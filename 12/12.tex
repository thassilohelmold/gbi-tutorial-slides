...
\begin{frame}{Wahr oder Falsch?}
	\begin{itemize}
		\truefalseQ{1}{myGreen}{$x^4 \in \Oh{{(x^3)}^3}$}{W $\qquad {(x^3)}^3 = x^9$}
		\truefalseQ{3}{myRed}{$\sqrt{n} \in \Om{2^n}$}{F}
		\truefalseQ{5}{myGreen}{$log_{5000} n \in \Th{\log_2{n^4}}$}{W $\qquad \log_2{n^4} = 4 \cdot \log_2{n}$}
		\truefalseQ{7}{myRed}{$O(f_1) + f_2 = O(f_1 + f_2)$}{F \newline Aber: $O(f_1) + O(f_2) = O(f_1 + f_2)$}
	\end{itemize}
\end{frame}

\begin{frame}{Wahr oder Falsch?}
	\begin{itemize}
		\truefalseQ{1}{myRed}{Für zwei Funktionen $f, g$ gilt immer $f \preceq g$ oder $f \succeq g$}{F}
	\end{itemize}
	\medskip
